...
export class WorkerModel {
    static async getAll() {
        try {
            ...
            throw new Error("Error al obtener todos los trabajadores desde la base de datos");
        }
    }
    ...
    static async getById({ idWorker }) {
        try {
            const [worker] = await connection.query(
                'SELECT * FROM trabajador WHERE id_trabajador = ?;', [idWorker]
            );
            if (worker.length === 0) {
                return [];
            }
            ...
            throw new Error("Error al obtener el trabajador por ID desde la base de datos");
        }
    }
    ...
    static async getWorkersByIdFarmer({ idFarmer }) {
        try {
            ...
            throw new Error("Error al obtener los trabajadores por ID de agricultor desde la base de datos");
        }
    }
